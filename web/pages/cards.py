...
def widgets_card():
    st.page_link("web/pages/widgets.py", label="Widgets", icon=":material/widgets:")
...
def text_card():
    st.page_link("web/pages/text.py", label="Text", icon=":material/article:")
...
def dataframe_card():
    st.page_link("web/pages/data.py", label="Data", icon=":material/table:")
    st.dataframe(st.session_state.chart_data, height=220)

def charts_card():
    st.page_link("web/pages/charts.py", label="Charts", icon=":material/insert_chart:")
    st.bar_chart(st.session_state.chart_data, height=230)

def media_card():
    st.page_link("web/pages/media.py", label="Media", icon=":material/image:")
    st.video("https://s3-us-west-2.amazonaws.com/assets.streamlit.io/videos/hero-video.mp4", autoplay=True)

def layouts_card():
    st.page_link("web/pages/layouts.py", label="Layouts", icon=":material/dashboard:")
...
def chat_card():
    st.page_link("web/pages/chat.py", label="Chat", icon=":material/chat:")
    st.chat_message("user").write("Hello, world!")
    st.chat_message("assistant").write("Hello, user!")
    st.chat_input("Type something")

def status_card():
    st.page_link("web/pages/status.py", label="Status", icon=":material/error:")
    cols = st.columns(2)
    cols[0].error("Error")
