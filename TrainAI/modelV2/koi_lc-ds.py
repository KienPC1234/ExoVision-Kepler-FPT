...
"""
combine_koi_lightcurves_parquet_v2.py
- Đọc dataset/koi_cumulative.csv
- Tải lightcurves cho mỗi kepid (Kepler) bằng lightkurve
- Ghép (stitch) tất cả quarter, chuẩn hóa 'time' column
- Ưu tiên cột flux: pdcsap_flux > sap_flux > flux
- Ghi kết quả (time, flux, kepid, label) vào dataset/koi_lightcurves.parquet
- Mã hóa nhãn:
    FALSE POSITIVE -> 0
    CANDIDATE     -> 1
    CONFIRMED     -> 2
...
backup_count = 3  # giữ tối đa 3 file log cũ
logging.basicConfig(
    level=logging.INFO,
